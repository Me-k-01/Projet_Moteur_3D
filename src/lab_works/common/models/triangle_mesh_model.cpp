...
namespace M3D_ISICG
{
	...
	void TriangleMeshModel::uniformSetUp(const GLuint p_glProgram)
	{
		for ( size_t i = 0; i < _meshes.size(); i++ )
		{
			_meshes[ i ].uniformSetUp(p_glProgram);
		}
	}
	...
	Texture TriangleMeshModel::_loadTexture( const aiString & p_path, const std::string & p_type )
	{
		...
		if ( image.load( fullPath ) )
		{
			...
			glTextureParameteri( texture._id, GL_TEXTURE_MIN_FILTER, GL_LINEAR );
