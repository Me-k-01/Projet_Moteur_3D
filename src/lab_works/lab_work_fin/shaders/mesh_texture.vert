...
// --- Uniforme
uniform vec3 uLightsPos[NUM_LIGHTS];
...
out vec3 vLightsPos[NUM_LIGHTS]; 
out vec3 vPosition;
out vec3 vNormal; 
...
void main()
{
	...
	if (uHasNormalMap) {
		...
		for (int i = 0; i < NUM_LIGHTS ; i++) {
			vLightsPos[i] = inv_TBN * uLightsPos[i]; // Position des lumières dans le tangent space
		}
	} else {
		// Position dans le view space du point sur la surface 
		vPosition = position;	
		vLightsPos = uLightsPos; 
	}
}
